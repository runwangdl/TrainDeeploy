...
[ -x "$ELF" ] || {
	echo "ERROR: binary not found: $ELF (generate+build the test first)"
	exit 1
}
...
[ -n "$CMD" ] || {
	echo "ERROR: could not extract gapy gvsoc command from $GVMAKE"
	exit 1
}
...
prevsz=-1
stall=0
STALL_NEEDED=12 # ~60s of no new pe trace => pe core idle/hung
...
while kill -0 $GVPID 2>/dev/null; do
	...
	[ "$sz" -eq "$prevsz" ] && stall=$((stall + 1)) || stall=0
	prevsz=$sz
	echo "[$(date +%H:%M:%S)] $((sz / 1048576))MB cyc=${cyc:-?} c.unimp=${hit} stall=${stall}/${STALL_NEEDED} free=${free_gb}G"
	if [ "$hit" -gt 0 ]; then
		reason="PC-CORRUPTION (c.unimp trap loop) — crash site below"
		kill -9 $GVPID 2>/dev/null
		pkill -9 gvsoc_launcher
		break
	fi
	if grep -qa "Invalid fetch" "$OUT" 2>/dev/null; then
		reason="FC Invalid fetch — GVSoC aborted, cluster trace lost (try a build that crashes cluster-side)"
		kill -9 $GVPID 2>/dev/null
		pkill -9 gvsoc_launcher
		break
	fi
	if [ "$stall" -ge "$STALL_NEEDED" ]; then
		reason="$CORE STALLED at cyc ${cyc} — this core stopped executing (idle during a master/optimizer phase, OR a hang). Trace another core / run the full training to tell which."
		kill -9 $GVPID 2>/dev/null
		pkill -9 gvsoc_launcher
		break
	fi
	if [ -n "$free_gb" ] && [ "$free_gb" -lt 8 ]; then
		reason="ABORT: low disk"
		kill -9 $GVPID 2>/dev/null
		pkill -9 gvsoc_launcher
		break
	fi
