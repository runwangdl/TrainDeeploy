...
while kill -0 $GVPID 2>/dev/null; do
	...
	if grep -qa "Invalid fetch\|Invalid access" "$OUT" 2>/dev/null; then
		reason="Invalid fetch/access — core aborted"
		kill -9 $GVPID 2>/dev/null
		pkill -9 gvsoc_launcher
		break
	fi
	if [ "$stall" -ge "$STALL_NEEDED" ]; then
		reason="$CORE STALLED at cyc ${cyc} (idle/hung)"
		kill -9 $GVPID 2>/dev/null
		pkill -9 gvsoc_launcher
		break
	fi
...
if [ -n "$LN" ]; then
	...
	for pc in $(sed -n "$((LN - 14)),$((LN - 1))p" "$OUT" | sed -E 's/\x1b\[[0-9]*m//g' | grep -oE ' [0-9a-f]{6,8} ' | awk '{print $1}' | tail -14); do
		printf "0x%s -> " "$pc"
		"$A2L" -f -e "$ELF" "0x$pc" 2>/dev/null | tr '\n' ' '
		echo
	done
fi
