    ...
    def addGeometricalConstraint(tilerModel: TilerModel, parseDict: Dict, ctxt: NetworkContext) -> TilerModel:
        ...
        for idx in range(4):
            tilerModel.addConstraint(
                tilerModel.getTensorDimVar(tensorName = data_in_name, dimIdx = idx) == tilerModel.getTensorDimVar(
                    tensorName = data_out_name, dimIdx = idx))

        # Per-channel vectors: single dimension follows C (dim 1 of data_in)
        for vec_name in [
                scale_name, bias_name, running_mean_name, running_var_name, saved_mean_name, saved_inv_std_name
        ]:
            tilerModel.addConstraint(
                tilerModel.getTensorDimVar(tensorName = vec_name, dimIdx = 0) == tilerModel.getTensorDimVar(
                    tensorName = data_in_name, dimIdx = 1))
...
class ChannelNormalizeTileConstraint(TileConstraint):
    ...
    def serializeTilingSolution(
        ...
        replacement_types = {
            "C": PointerClass(uint16_t),
            "H_in": PointerClass(uint16_t),
...
class BNGradReduceTileConstraint(TileConstraint):
    ...
    def addGeometricalConstraint(tilerModel: TilerModel, parseDict: Dict, ctxt: NetworkContext) -> TilerModel:
        ...
        for idx in range(4):
            tilerModel.addConstraint(
                tilerModel.getTensorDimVar(tensorName = dY_name, dimIdx = idx) == tilerModel.getTensorDimVar(
                    tensorName = X_name, dimIdx = idx))

        # Per-channel vectors follow C
        for vec_name in [saved_mean_name, saved_inv_std_name, dgamma_name, dbeta_name]:
            tilerModel.addConstraint(
                tilerModel.getTensorDimVar(tensorName = vec_name, dimIdx = 0) == tilerModel.getTensorDimVar(
                    tensorName = dY_name, dimIdx = 1))
...
class BNGradNormalizeTileConstraint(TileConstraint):
    ...
    def addGeometricalConstraint(tilerModel: TilerModel, parseDict: Dict, ctxt: NetworkContext) -> TilerModel:
        ...
        for name in [
                dY_name, X_name, saved_mean_name, saved_inv_std_name, gamma_name, dgamma_name, dbeta_name, dX_name
        ]:
            tilerModel.addTensorDimToModel(ctxt, name)
...
class BatchNormalizationGradTileConstraint(TileConstraint):
    ...
    def addGeometricalConstraint(tilerModel: TilerModel, parseDict: Dict, ctxt: NetworkContext) -> TilerModel:
        ...
        for name in [
                dY_name, X_name, gamma_name, saved_mean_name, saved_inv_std_name, dX_name, dgamma_name, dbeta_name
        ]:
            tilerModel.addTensorDimToModel(ctxt, name)
