...
def solveSchedule(order, mkb, ban_csv, tl, floor_kb = 0, EXACT = False, seqPath = None, graphSource = ''):
    """Solve one rematerialisation problem over `order` and return the emitted schedule.

    Split out of the module body so the same solver can be applied to a SUBGRAPH, which
    is what a Rockmate-style two-level search needs: an inner solve per repeated block
    producing several (peak, cycles) options, then a choice across blocks. Whole-graph
    solving is that with one block.

    `order` is the executed order, not the graph: stage t is the stage that first
    produces node t, so the ordering is an INPUT to the formulation and whatever memory
    the ordering alone can remove must already have been removed.

    Returns the dict written to `seqPath`, or None when the model has no solution.
    """
    ...
    sizes = [
        0 if _i in _aliased else sum(sz(_o) for _o in _nd.outputs if _o is not None) for _i, _nd in enumerate(order)
    ]
    ...
    if mkb == 0:
        ...
        print('MODELPEAK=%d' %
              max(sum(b for q, (_, _p, b) in enumerate(tensors) if _p <= t <= _tl[q]) for t in range(n)))
    ...
    if os.environ.get('FEAS_ONLY') == '1':
        ...
        if _min_peak:
            ...
            model.Minimize(obj)
    # Behaviour-preservation probe: hash the constructed model and exit before Solve.
    # The refactor moved code without touching the formulation, so the CP-SAT model must
    # be byte-identical. Comparing SOLUTIONS instead would compare CP-SAT under a
    # wall-clock limit, which is nondeterministic even at WORKERS=1 and varies with load.
    if os.environ.get('DUMP_MODEL_HASH'):
        import hashlib as _h
        _blob = model.Proto().SerializeToString()
        print(f'MODELHASH={_h.sha1(_blob).hexdigest()[:16]} bytes={len(_blob)}')
        raise SystemExit(0)
    ...
    json.dump(
        ...
        }, open(seqPath or os.environ.get('SEQ_PATH', 'recompute_checkmate.json'), 'w'))
...
def main():
    ...
    # Default OFF. EXACT=1 raises below -- the Free[t,i,k] recurrence it needs is written
    # over node-indexed retention and was never ported to the tensor-indexed S, and it
    # does not solve at this graph size anyway. Defaulting it ON made the documented
    # invocation exit immediately, which is a poor way to learn that.
    exact = os.environ.get('EXACT', '0') == '1'

    graph = gs.import_onnx(onnx.load(dep))
    if base_name not in ('mem_min_schedule', 'memory_minimising'):
        raise SystemExit(f'unknown base schedule {base_name!r}; the repository provides '
                         'the memory-minimising list schedule only. Pass "mem_min_schedule".')
    order = [grp[0] for grp in _memoryMinimisingScheduler(graph)]
    solveSchedule(order, mkb, ban_csv, tl, floor_kb, exact, os.environ.get('SEQ_PATH', 'recompute_checkmate.json'),
                  os.path.abspath(dep))
